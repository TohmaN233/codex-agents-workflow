...
export async function createStrictSession(options) {
  ...
  const profile = await buildCodexProfile(options); let client; let policy; let launch; let authAccountId = null; let activeThread = null; let activeTurn = null; let closed = false; let used = false; let closing;
  try {
  ...
  const session = {
    profile, get client() { return client; },
    ...
    async turn(text, { explicit_sources = [], timeout_ms = 60000 } = {}) {
      ...
      if (!profile.model_catalog_sha256) {
        ...
        await client.close(); await launch();
        const observed = await authenticatedModel(client, model, effort, options.onModelCatalog);
        await client.close(); await pinObservedModel(profile, observed); await launch();
        await policy.apply(client);

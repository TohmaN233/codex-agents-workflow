...
export function validateStrictConfig(raw = {}) {
  ...
  const result = { enabled: raw.enabled ?? false, codex_binary: raw.codex_binary ?? '', binary_sha256: raw.binary_sha256 ?? '',
    ...
    main_model: raw.main_model ?? 'gpt-6-astra', main_reasoning_effort: raw.main_reasoning_effort ?? 'medium' };
  ...
  requireValue(typeof result.main_model === 'string' && /^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$/.test(result.main_model) &&
    typeof result.main_reasoning_effort === 'string' && /^[a-z][a-z0-9_-]{0,31}$/.test(result.main_reasoning_effort), 'STRICT_CONFIG', 'Finalizer model and reasoning effort must be explicit nonempty identifiers; executor availability is checked at launch');

...
export function createWorkflowPreset(presetId, providers, routingRules) {
  ...
  const selectProvider = (route, label) => {
    ...
    requireValue(provider?.enabled && provider.kind==='native_agent' && provider.capabilities?.read && ['implementer','advisor'].includes(provider.config?.role),
      'PRESET_PROVIDER_UNAVAILABLE',`Configure an enabled native ${label} Provider before adding this preset`,{provider_id:providerId,route});
    const conflict=nativeBindingIssue(provider);
    requireValue(!conflict,conflict?.code,conflict?.message,conflict?{binding:conflict}:{});
    return provider;
  };
  const planningProvider=selectProvider('planning','planning');
  const productionProvider=selectProvider(image?'complex_implementation':'implementation',image?'image-production':'production');

...
export function expansionRunPack(pack, resources, provider, id, routingRules, automatic = false, reviewer = null, providers = []) {
  ...
  return { workflow, resources: planningResources,
    provenance: { kind: 'skill_expansion_job', ...(generation ? {generation} : {}), source_workflow_id: pack.workflow.id, source_revision: pack.revision_hash, selected_provider_id: provider.id, ...(routingRules ? { routing_rules: structuredClone(routingRules), routing_catalog:packet.routing_catalog } : {}) },
    import_report: null };
}

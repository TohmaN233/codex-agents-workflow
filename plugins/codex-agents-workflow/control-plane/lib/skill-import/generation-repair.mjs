...
export async function repairGeneration(runtime,args,record,feedback) {
  ...
  if (state.nodes.expand.attempts.length>=policy.max_rounds) return {phase:'attention',error:{code:'GENERATION_REPAIR_LIMIT',message:'自动修正已达到轮数上限，保留全部结果供检查。'},feedback};
  ...
  await runtime.transition(args.run_id,'generation_repair',(current,definition)=>{
    ...
    current.generation_repair={round:current.nodes.expand.attempts.length+1,feedback,previous_proposal:state.nodes.expand.output};
    advanceRun(current,definition);current.updated_at=new Date().toISOString();
  },{expected_sequence:sequence});
  return {phase:'repairing',round:state.nodes.expand.attempts.length+1,feedback};
}

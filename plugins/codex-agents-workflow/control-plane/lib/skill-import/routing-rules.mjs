...
export function validateRoutingRules(rules) {
  requireValue(rules && rules.version === 1 && typeof rules.instructions === 'string' && rules.instructions.trim() && rules.instructions.length <= 16000 && rules.routes && Object.keys(rules).every(k => ['version','instructions','routes','generation'].includes(k)), 'ROUTING_RULES', 'Routing rules require version 1, instructions and routes');
...
export function validateGenerationSettings(value = {}) {
  requireValue(value && Object.keys(value).every(k=>['review_provider_id','max_rounds'].includes(k)), 'GENERATION_SETTINGS', 'Unknown generation setting');

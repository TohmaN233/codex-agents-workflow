...
export function SkillGeneration({pack,routing,workspace,saved,openRun}: {pack:Json,routing:Json|null,workspace:string,saved:(pack:Json)=>Promise<void>,openRun:(run:Json)=>void}) {
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      try { const next=await api('advance_authoring',{run_id:run.run_id,control_token:run.control_token}); if(active){setProgress(next); if(!['review_required','ready_to_apply','attention','approval','authentication_required'].includes(next.phase)) timer=setTimeout(poll,2000);} }
  ...
  async function start(){
    ...
    try {const result=await api('start_authoring',{workflow_id:pack.workflow.id,revision_hash:pack.revision_hash,run_id:uid('authoring'),...(routing?{routing_rules:routing}:{}),...(workspace?{workspace}:{})});rememberRun(result);sessions.set(sessionKey,result);setRun(result);}
    catch(cause){fail(cause);}finally{setBusy(false);}
  }
  async function accept(){setBusy(true);try{const result=await api('accept_authoring',{...control,accepted:true});sessions.delete(sessionKey);await saved(result);}catch(cause){fail(cause);}finally{setBusy(false);}}
  ...
  return <section className="generation-panel"><h2>{t('从 Skill 生成 Workflow（自动选择）','Generate a workflow from a Skill (automatic routing)')}</h2><p>{t('自动安排步骤，在 Main 与 Codex task thread 之间分配执行者并检查结果。工作目录由系统准备，通常无需设置。','Automatically arranges steps, assigns executors between Main and Codex task threads, and reviews the result. The system prepares the workspace, so setup is usually unnecessary.')}</p>
    ...
    {progress?.phase==='authentication_required' && <div><p>{t('此操作仅用于你主动选择的独立登录模式。完成登录后继续。','This only applies to the managed login mode you selected. Finish signing in, then continue.')}</p>{progress.live?.status==='auth_required' && <button disabled={busy} onClick={async()=>{setBusy(true);try{const value=await api('login_authoring',control!);setLogin(value);setProgress({...progress,live:value});}catch(cause){fail(cause);}finally{setBusy(false);}}}>{t('获取官方登录链接','Get the official sign-in link')}</button>}{login?.auth_url && <a href={login.auth_url} target="_blank" rel="noreferrer">{t('打开官方登录页面','Open the official sign-in page')}</a>}<button onClick={()=>{setError(null);setProgress(null);}}>{t('登录完成，继续','Sign-in complete; continue')}</button></div>}

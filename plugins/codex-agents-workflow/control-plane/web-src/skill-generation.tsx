...
export function SkillGeneration({pack,routing,provider,workspace,saved,openRun}: {pack:Json,routing:Json|null,provider:string,workspace:string,saved:(pack:Json)=>Promise<void>,openRun:(run:Json)=>void}) {
  ...
  useEffect(()=>{
    if (!run || error || ['review_required','ready_to_apply','attention','approval','authentication_required'].includes(progress?.phase)) return;
  ...
  return <section className="generation-panel"><h2>从 Skill 自动生成工作流</h2><p>自动安排步骤、按规则选择执行者并检查结果。工作目录由系统准备，通常无需设置。</p>
    ...
    {progress?.phase==='review_required' && <><h3>生成的步骤</h3><ol>{progress.workflow?.nodes?.filter((node:Json)=>node.executor).map((node:Json)=><li key={node.id}>{node.name || node.id}{' — '+(node.executor.kind==='provider'?node.executor.provider_id:node.executor.kind==='main'?'主 Agent 最终确认':node.executor.kind==='human'?'用户确认':node.executor.kind)}{node.access?'（'+(node.access==='read_only'?'只读':node.access)+'）':''}</li>)}</ol><p>结构检查：{progress.validation?.valid?'通过':'未通过'}；最终确认：{progress.workflow?.finalization?.required?'必须完成':'未要求'}。</p><h3>检查意见</h3><pre>{typeof progress.review?.structured_output==='string'?progress.review.structured_output:JSON.stringify(progress.review?.structured_output,null,2)}</pre><Details title="查看完整草稿和检查证据" value={{workflow:progress.workflow,validation:progress.validation,proposal:progress.proposal,review:progress.review}}/></>}

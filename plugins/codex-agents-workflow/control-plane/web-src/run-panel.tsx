  ...
  <div className="run-body"><Canvas workflow={pack.workflow} runtime={state.nodes} onChange={() => {}} onSelect={(kind,id) => { if (kind === 'node') setNodeId(id); }} readOnly/>
    <aside className="inspector scroll"><h2>{nodeId || '运行详情'}</h2>{!nodeId && <p>选择节点查看状态、产物、证据及可用操作。</p>}
      ...
      {current && <><Status value={current.status}/><Details title="节点输出、证据和诊断" value={current}/>
        ...
        {live && <><p role="status">Strict 会话：{live.status}</p>{live.error && <Details title="会话诊断" value={live.error}/>} {['claimed','running'].includes(current.status) && live.status !== 'closed' && <details open={!!live.output_preview?.text}><summary>实时输出预览 · 尚未验收</summary><pre>{live.output_preview?.text || '尚无输出'}</pre>{live.output_preview?.truncated && <small>仅展示最近 32K 字符；最终结果以持久化产物为准。</small>}</details>}</>}
        ...
        {lease && token && ['claimed','running'].includes(current.status) && <>
          ...
          {attempt?.dispatch && <>
            {pack.workflow.skill_policy.mode === 'strict' && definition.executor.kind !== 'subworkflow' && <><button onClick={() => call('strict_status')}>读取 Strict 会话状态</button>{live?.show_login && <button onClick={() => act(async () => setLogin({ attempt_id: attempt.id, value: await api('strict_login', args) }))}>获取此会话的官方登录链接</button>}{definition.role === 'finalizer' && <label className="check"><input type="checkbox" checked={accepted} onChange={e => setAccepted(e.target.checked)}/>我已审查并接受主节点提案</label>}<button onClick={() => call('collect_strict', { accepted })}>收集固定 Strict 结果</button></>}
      ...
      {currentLogin && <Details title="此会话登录信息" value={currentLogin}/>}
      {currentLogin && Object.entries(currentLogin).filter(([key,value]) => /url/i.test(key) && typeof value === 'string' && /^https?:\/\//.test(value)).map(([key,value]) => <a key={key} href={String(value)} target="_blank" rel="noreferrer">打开官方登录页面</a>)}

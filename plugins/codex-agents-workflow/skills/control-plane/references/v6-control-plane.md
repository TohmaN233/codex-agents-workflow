...
Read this contract only when codex_agents_workflow_status reports version6. For version7,
use the Workflow Run protocol in the skill entrypoint.


# Codex Agents Workflow Plane

Act as the primary architect. Keep requirements, architecture, route selection, scope,
verification, escalation, and final acceptance in the root session. A
...
Before repository/task tools, call `codex_agents_workflow_status` at most once. It returns only
...
Call `codex_agents_workflow_resolve` once for the selected Task Type. It returns the fixed ordered
...
For a built-in connector, call `codex_agents_workflow_connector_start`; it resolves and delivers the
...
- `codex_agents_workflow_connector_probe` — transport/configuration probe; it sends no task;
- `codex_agents_workflow_connector_start` — starts one exact task and returns identity;
- `codex_agents_workflow_connector_status` — reads one exact `task_id`, with bounded wait;
- `codex_agents_workflow_connector_control` — `reconcile`, exact cancellation, permission/input response,
...
Call `codex_agents_workflow_invoke` only for an enabled read-only OpenAI-compatible provider when

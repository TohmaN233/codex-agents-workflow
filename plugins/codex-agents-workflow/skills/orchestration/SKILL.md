...
Preflight only an auxiliary selected
by the declared route: none for solo; Luna / Max or Terra / High for delegate; fresh
Astra / Medium for audit; and the selected implementer plus fresh Astra reviewer for full.
...
- `audit`: root implements and verifies; a fresh read-only Astra / Medium reviewer reviews
  the accumulated diff; spawn no implementer.
- `full`: only for an explicit broad or high-risk exception. Select one implementer,
  root verifies, then a fresh read-only Astra / Medium reviewer reviews.
...
For `audit` and `full`, after parent verification, spawn a new native Astra / Medium

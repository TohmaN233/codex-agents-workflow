...
This is the maintainer and operator reference for Codex Agents Workflow's native custom-agent
...
Use a fresh Astra reviewer only for audit or full after parent verification:
...
Companion installation is separate
...
node plugins/codex-agents-workflow/scripts/install-agents.mjs --check
~~~
...
| audit | `--check --check-role reviewer` |
| full (Luna) | `--check --check-role luna --check-role reviewer` |
| full (Terra) | `--check --check-role terra --check-role reviewer` |

For example:

~~~text
node plugins/codex-agents-workflow/scripts/install-agents.mjs --check --check-role luna
node plugins/codex-agents-workflow/scripts/install-agents.mjs --check --check-role reviewer
~~~
...
for higher-risk delegate/full implementation, and Astra / medium for audit/full review.
...
fresh Astra reviewer reviews. In full, one selected implementer completes the spec, the
root verifies, and a fresh Astra reviewer reviews. Auxiliary work substitutes for root

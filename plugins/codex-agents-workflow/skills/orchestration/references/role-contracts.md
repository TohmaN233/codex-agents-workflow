...
Use these contracts with Codex Agents Workflow's namespaced, role-pinned native custom agents.
...
Preflight only auxiliaries selected by the route: none for solo; Luna / Max or
Terra / High for delegate; fresh Astra / Medium for audit; and one selected implementer
plus fresh Astra reviewer for full. Cache each successful check only for the task. After
...
2. Observe the selected role, model, and effort through public spawn/details metadata
   ...
   delegate/full implementation, and Astra / Medium for audit/full review.
...
- `audit`: root implements and verifies. A fresh read-only Astra / Medium reviewer inspects
...
- `full`: use only for an explicit broad or high-risk exception. One selected Luna /
  ...
  verifies, and a fresh read-only Astra / Medium reviewer inspects the accumulated diff.
...
~~~text
ROLE
Act as Codex Agents Workflow's default routine implementation worker. Execute the supplied
...
~~~text
ROLE
Act as Codex Agents Workflow's explicit high-complexity escalation worker. Resolve the supplied
specification within the settled architecture, preserve every stated interface and
constraint, and surface ambiguity instead of redesigning the architecture.
...
## Fresh Astra / Medium - requested-read-only audit/full reviewer
...
The installed role pins Astra / Medium and requests a read-only sandbox. Do not attach
...
The bundled reviewer reviewing its own workflow is context-clean, not cross-model-family independence.

...
[ "$(jq -r '.version | split("+")[0]' "$manifest")" = 0.8.0 ] || fail "native manifest version drifted"
...
jq -e '.name == "codex-agents-workflow" and (.plugins[] | select(.name == "codex-agents-workflow"))' "$marketplace" >/dev/null || fail "local marketplace still exposes the retired sol-advisor identity"

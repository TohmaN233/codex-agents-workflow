...
async function check(label){const t=await call('thread/start',{cwd:base,ephemeral:true});const s=await call('mcpServerStatus/list',{threadId:t.thread.id});assert.equal(s.data.find(x=>x.name==='codex-agents-workflow').runtimeStatus,'connected');console.log(label,'connected');}
await check('BEFORE');

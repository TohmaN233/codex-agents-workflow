...
import { createStrictSession } from '../../plugins/codex-agents-workflow/control-plane/lib/execution/codex-session.mjs';
import { StrictSessionManager } from '../../plugins/codex-agents-workflow/control-plane/lib/execution/strict-session-manager.mjs';
import { STRICT_INSTRUCTIONS } from '../../plugins/codex-agents-workflow/control-plane/lib/execution/codex-profile-builder.mjs';
import { WorkflowService } from '../../plugins/codex-agents-workflow/control-plane/lib/workflow-service.mjs';
import { loadConfig, saveConfig } from '../../plugins/codex-agents-workflow/control-plane/lib/config.mjs';
...
import { DEFAULT_CONFIG_PATH } from '../../plugins/codex-agents-workflow/control-plane/server.mjs';
import { sourceManifest } from './source-manifest.mjs';
import { discoverCodexSkills } from '../../plugins/codex-agents-workflow/control-plane/lib/skill-import/codex-inventory.mjs';
...
try {
  ...
  for (const path of ['strict-session-manager', 'strict-config', 'codex-managed-login']) {
    const relative = `../../plugins/codex-agents-workflow/control-plane/lib/execution/${path}.mjs`;
  ...
  for (const name of ['workflow-service', 'workflow-executor', 'workflow-runtime', 'workflow-run-store', 'workflow-execution-envelope', 'workflow-state', 'workflow-validator', 'workflow-pins', 'workflow-subworkflow', 'workflow-reference-schema', 'workflow-events', 'config']) {
    const relative = `../../plugins/codex-agents-workflow/control-plane/lib/${name}.mjs`;
    report.source_hashes[relative] = digest(await readFile(fileURLToPath(new URL(relative, import.meta.url))));
  }
  for (const name of ['codex-inventory', 'expansion-run', 'semantic-expander', 'metadata-reader', 'coarse-compiler', 'dependency-reader', 'review-import', 'skill-reader', 'inline-skill']) {
    const relative = `../../plugins/codex-agents-workflow/control-plane/lib/skill-import/${name}.mjs`;
    report.source_hashes[relative] = digest(await readFile(fileURLToPath(new URL(relative, import.meta.url))));
  }
